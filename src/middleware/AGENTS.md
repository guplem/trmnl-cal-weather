...
- `?src=cal` adds `data.cache_write_failed` when the `CacheService` put failed (usually a payload over the 100 KB value limit). The cache then never warms and every poll pays the ~15s live build, so the template shows it as a diagnostic.
